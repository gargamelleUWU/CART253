...
let missedFlies = 0;
let combo = 0;
// Note: Size variables (blueSize, etc.) have been moved into the selectButton object.
...
const selectButton = {
    ...
    color2: ("#0b4553ff"),
    state: "null",
    // Grouped all select-screen-only variables here
    blueSize: 100,
    redSize: 100,
    greenSize: 100,
    yellowSize: 100,
...
function setup() {
    ...
    // Set drawing modes once for efficiency
    imageMode(CENTER);
    textAlign(CENTER);
    textFont('impact');
...
function drawFly() {
    changeFly();
    push();
    // imageMode(CENTER) removed; set in setup()
...
function drawTutorial() {
    ...
    // textFont('impact') removed; set in setup()
    textSize(45);
    text("PLAY", 160, 60);
    text("SELECT", 440, 60);
    pop();
    ...
    push();
    fill("#605a7cff");
    // textAlign(CENTER) removed; set in setup()
    // textFont('impact') removed; set in setup()
    textSize(45);
...
function resetTongue() {
    ...
    frog.tongue.y = 560;
    frog.tongue.state = "idle"; // CRITICAL FIX: Resets the tongue's state
}
 ...
 */
function drawFrog() {
    // Combined three push/pop pairs into one for efficiency
    push();
    ...
    if (Hanez) {
        ...
        // Use a nested push/pop here just for the fallback ellipse
        push();
        fill("#20c392ff");
        stroke("#044628ff");
...
function mousePressed() {
    if (frog.tongue.state === "idle") {
        ...
        // OPTIMIZATION: Check if sound is playing before playing it
        if (!tongueLaunch.isPlaying()) {
    ...
    if (gameState === "select") {
        // References updated to use selectButton.blueSize, .redSize, etc.
        if ((mouseX > (350 - selectButton.blueSize / 2) && mouseX < (350 + selectButton.blueSize / 2)) && (mouseY > (200 - selectButton.blueSize / 2) && mouseY < (200 + selectButton.blueSize / 2)))
            Hanez = frogBlue;
        if ((mouseX > (550 - selectButton.redSize / 2) && mouseX < (550 + selectButton.redSize / 2)) && (mouseY > (200 - selectButton.redSize / 2) && mouseY < (200 + selectButton.redSize / 2))) {
            Hanez = frogRed;
        }
        if ((mouseX > (350 - selectButton.greenSize / 2) && mouseX < (350 + selectButton.greenSize / 2)) && (mouseY > (400 - selectButton.greenSize / 2) && mouseY < (400 + selectButton.greenSize / 2))) {
            Hanez = frogGreen;
        }
        if ((mouseX > (550 - selectButton.yellowSize / 2) && mouseX < (550 + selectButton.yellowSize / 2)) && (mouseY > (400 - selectButton.yellowSize / 2) && mouseY < (400 + selectButton.yellowSize / 2))) {
            Hanez = frogYellow;
        }

        if (mouseX > (350 - selectButton.tong1Size / 2) && mouseX < (350 + selectButton.tong1Size / 2) && mouseY > (60 - selectButton.tong1Size / 2) && mouseY < (60 + selectButton.tong1Size / 2)) {
            frog.tongue.color = ("#f03737ff");
        }
        if (mouseX > (450 - selectButton.tong2Size / 2) && mouseX < (450 + selectButton.tong2Size / 2) && mouseY > (60 - selectButton.tong2Size / 2) && mouseY < (60 + selectButton.tong2Size / 2)) {
            frog.tongue.color = ("#f153c7ff");
        }
        if (mouseX > (550 - selectButton.tong3Size / 2) && mouseX < (550 + selectButton.tong3Size / 2) && mouseY > (60 - selectButton.tong3Size / 2) && mouseY < (60 + selectButton.tong3Size / 2)) {
            frog.tongue.color = ("#ff7a0eff");
        }
    }

    // This is your logic for the back button on select/end screens
    if (gameState === "select" || gameState === "end") {
        if (mouseX > 15 && mouseX < 70 && mouseY > 20 && mouseY < 60) {
            gameState = "start";
            // No resetTongue() needed here, because endScreen() or
            // controlState() (for 'select') already handled it.
        }
    }
}
...
function selectScreen() {
    // This is your custom logic for the select screen tongue position
    frog.tongue.x = frog.body.x;
    frog.tongue.y = 200;
    // OPTIMIZATION: These functions do nothing here because
    // tongue.y is 200 (not 0), so they were removed.
    // detectTongue();
    // tongueButtonOverlay();
...
function endScreen() {
    ...
    frog.body.size = 75;
    resetFly();
    resetTongue(); // This now correctly sets state to "idle"
    resetHunger();
    drawEndScreen();
}
...
function controlState() {
    if (gameState === "start") {
        ...
        if (selectButton.state === "play") {
            // Your check for the soundtrack is good!
        ...
        if (missedFlies === missLimit) {
            applyWaterFilter();
            gameState = "end";
            // endScreen() will call resetTongue() on the next frame
        }
...
function debug() {
    // OPTIMIZATION: Commented out to prevent console spam
...
function drawHunger() {
    ...
    fill(255);
    // textFont('Impact') removed; set in setup()
    // We must override the global textAlign(CENTER) here
...
function drawLilypad() {
    // push() and pop() removed; they were redundant
    // imageMode(CENTER) removed; set in setup()
...
function drawFrogScreen() {
    ...
    // Checks that the image for Hanez is loaded, if so it uses it
    if (frogBlue) {
        image(frogBlue, 350, 200, selectButton.blueSize, selectButton.blueSize);
        // Alternate form for Hanez if the png is not found
    } else {
        push();
        fill("#20c392ff");
    ...
    if (frogRed) {
        image(frogRed, 550, 200, selectButton.redSize, selectButton.redSize);
        // Alternate form for Hanez if the png is not found
    } else {
        push();
        fill("#20c392ff");
    ...
    if (frogGreen) {
        image(frogGreen, 350, 400, selectButton.greenSize, selectButton.greenSize);
        // Alternate form for Hanez if the png is not found
    } else {
        push();
        fill("#20c392ff");
    ...
    if (frogYellow) {
        image(frogYellow, 550, 400, selectButton.yellowSize, selectButton.yellowSize);
        // Alternate form for Hanez if the png is not found
    } else {
        push();
        fill("#20c392ff");
    ...
    if ((mouseX > (350 - selectButton.blueSize / 2) && mouseX < (350 + selectButton.blueSize / 2)) && (mouseY > (200 - selectButton.blueSize / 2) && mouseY < (200 + selectButton.blueSize / 2))) {
        selectButton.blueSize = 125;
    } else {
        selectButton.blueSize = 100;
    }
    // Hover effect for red frog
    if ((mouseX > (550 - selectButton.redSize / 2) && mouseX < (550 + selectButton.redSize / 2)) && (mouseY > (200 - selectButton.redSize / 2) && mouseY < (200 + selectButton.redSize / 2))) {
        selectButton.redSize = 125;
    } else {
        selectButton.redSize = 100;
    }
    // Hover effect for green frog
    if ((mouseX > (350 - selectButton.greenSize / 2) && mouseX < (350 + selectButton.greenSize / 2)) && (mouseY > (400 - selectButton.greenSize / 2) && mouseY < (400 + selectButton.greenSize / 2))) {
        selectButton.greenSize = 125;
    } else {
        selectButton.greenSize = 100;
    }
    // Hover effect for yellow frog
    if ((mouseX > (550 - selectButton.yellowSize / 2) && mouseX < (550 + selectButton.yellowSize / 2)) && (mouseY > (400 - selectButton.yellowSize / 2) && mouseY < (400 + selectButton.yellowSize / 2))) {
        selectButton.yellowSize = 125;
    } else {
        selectButton.yellowSize = 100;
    }
}
...
function tongueColors() {
    push();
    rectMode(CENTER);
    noStroke(); // Call noStroke() once

    fill("#f03737ff");
    rect(350, 60, selectButton.tong1Size, selectButton.tong1Size);

    // noStroke() removed
    fill("#f153c7ff");
    rect(450, 60, selectButton.tong2Size, selectButton.tong2Size);

    // noStroke() removed
    fill("#ff7a0eff");
    rect(550, 60, selectButton.tong3Size, selectButton.tong3Size);

    pop();

    if (mouseX > (350 - selectButton.tong1Size / 2) && mouseX < (350 + selectButton.tong1Size / 2) && mouseY > (60 - selectButton.tong1Size / 2) && mouseY < (60 + selectButton.tong1Size / 2)) {
        selectButton.tong1Size = 75;
    } else {
        selectButton.tong1Size = 50;
    }

    if (mouseX > (450 - selectButton.tong2Size / 2) && mouseX < (450 + selectButton.tong2Size / 2) && mouseY > (60 - selectButton.tong2Size / 2) && mouseY < (60 + selectButton.tong2Size / 2)) {
        selectButton.tong2Size = 75;
    } else {
        selectButton.tong2Size = 50;
    }

    if (mouseX > (550 - selectButton.tong3Size / 2) && mouseX < (550 + selectButton.tong3Size / 2) && mouseY > (60 - selectButton.tong3Size / 2) && mouseY < (60 + selectButton.tong3Size / 2)) {
        selectButton.tong3Size = 75;
    } else {
        selectButton.tong3Size = 50;
    }
}
